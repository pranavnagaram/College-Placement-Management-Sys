...
const AppliedToJob = async (req, res) => {
  try {
    ...
    // Validate Placement vs Internship rules
    if (job.opportunityType === 'Placement') {
      const studentYear = user?.studentProfile?.year;
      if (!studentYear || studentYear < 3) {
        return res.json({ msg: "Placements are strictly restricted to Pre-final (3rd) and Final (4th) year students." });
      }
    }
