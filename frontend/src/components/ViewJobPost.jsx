...
function ViewJobPost() {
  ...
  const handleUpdateStatus = async (studentId, status) => {
    try {
      const response = await axios.put(`${BASE_URL}/tpo/job/update-status`, 
      ...
      console.log("error while updating applicant status => ", error);
    }
  }
                              ...
                              <Table striped bordered hover size='sm' className='text-center'>
                                <thead>
                                  <tr>
                                    ...
                                    <th style={{ width: '15%' }}>Current Round</th>
                                ...
                                <tbody>
                                  {
                                    applicant?.length > 0 ? (
                                      <>
                                        {
                                          applicant.map((app, index) => (
                                            <tr key={index}>
                                              ...
                                              <td>
                                                  {currentUser.role === 'recruiter' && (app.status === 'applied' || app.status === 'pending') && (
                                                    <div className="flex justify-center gap-2">
                                                      <Button variant="success" size="sm" onClick={() => handleUpdateStatus(app.id, 'hired')}>Accept</Button>
                                                      <Button variant="danger" size="sm" onClick={() => handleUpdateStatus(app.id, 'rejected')}>Reject</Button>
                                                    </div>
                                                  )}
                                              </td>
                                            </tr>
                                      ...
                                      <tr>
                                        <td colSpan={7}>No Student Yet Applied!</td>
                                      </tr>
                                    )
                                  }
                                </tbody>
                    ...
                    <Accordion.Body>
                      <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col backdrop-blur-md bg-white/30 border border-white/20 rounded-lg px-2 shadow-sm shadow-red-400">
                          ...
                          <span className='py-3'>
                            <span className={`px-3 py-1 text-sm font-semibold rounded-full inline-block w-max ${data?.opportunityType === 'Internship' ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'}`}>
                              {data?.opportunityType || 'Placement'}
                            </span>
                          </span>
                        </div>
                        {/* job title  */}
