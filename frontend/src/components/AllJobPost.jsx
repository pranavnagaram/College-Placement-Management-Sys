...
function AllJobPost() {
    ...
    axios.get(`${BASE_URL}/user/detail`, {
      ...
      .then(res => {
        setCurrentUser({
          ...
          role: res.data.role,
          companyId: res.data.recruiterProfile?.companyId,
        });
  ...
  const filteredJobs = jobs?.filter(job => 
    currentUser?.role !== 'recruiter' || job.company === currentUser?.companyId
  );
      ...
      <div className=''>
        {
          loading || !currentUser ? (
            ...
            <div className="overflow-x-auto max-sm:text-sm max-sm:p-1">
              <div className="table-scrollbar rounded-xl border border-slate-200 shadow-sm overflow-hidden bg-white my-6">
                <Table hover className='w-full text-slate-700 mb-0'>
                      ...
                      filteredJobs?.map((job, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td>
                              <span className={`px-2 py-1 text-xs font-semibold rounded-full ${job?.opportunityType === 'Internship' ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'}`}>
                                {job?.opportunityType || 'Placement'}
                              </span>
                            </td>
                            <td>{job?.jobTitle}</td>
                            <td>{job?.salary}</td>
                            ...
                            <td>
                              <div className="flex justify-around items-center">
                                {
                                  (currentUser.role !== 'recruiter' || currentUser.companyId === job.company) && (
                                    <>
                                          ...
                                          <i
                                            ...
                                            onClick={() => {
                                                const rolePaths = {
                                                  ...
                                                  'recruiter': `../recruiter/job/${job._id}`,
                                                };
                                                ...
                                                <i
                                                  ...
                                                  onClick={() => {
                                                      const rolePaths = {
                                                        ...
                                                        'recruiter': `../recruiter/post-job/${job._id}`,
                                                      };
                                                    navigate(rolePaths[currentUser.role]);
                                                  }}
                                                />
